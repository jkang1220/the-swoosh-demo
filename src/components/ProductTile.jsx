...
class ProductTile extends React.Component {
	render() {
		...
		const { name, description, retail_price, product_sku, stock, img } = product;

		const ADD_ITEM_TO_USER_CART = gql`
		mutation {
		  AddItemToCart(input: {user_id: ${user_id}, cart_id: ${cart_id}, product_id: 1}) {
		    id
		    name
		  }
		}
				...
				<Card className={classes.card}>
					<Table>
						<TableBody>
							...
							<TableRow>
								<TableCell><b>Product SKU:</b> {product_sku} </TableCell>
							</TableRow>
							<TableRow>
								<TableCell><b>Current Stock Count:</b> {stock} </TableCell>
							</TableRow>
						</TableBody>
						<TableFooter>
							<TableRow>
