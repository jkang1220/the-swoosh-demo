...
const resolvers = {
  ...
  Order: {
    ...
    products: async parent => await knex.select('*').from('products').join('order_product', 'products.id', 'order_product.product_id').where('order_product.order_id', '=', parent.id),
  },
  Cart: {
    products: async parent => (await knex.raw(`select products.* from (SELECT carts.user_id, cart_product.product_id FROM carts INNER JOIN cart_product ON carts.id = cart_product.cart_id) as wow INNER JOIN products ON wow.product_id = products.id where user_id = ${parent.user_id}`))[0],
  },
  Query: {
    ...
    getOrdersByUserId: async (parent, args) => await knex.select().from('orders').where(args).first(),
    ...
    getAllCarts: async (parent, args) => await knex('cart').where(args).first(),
    getCartByUserId: async (parent, args) => await knex('cart').where(args).first(),
  },
  Mutation: {
